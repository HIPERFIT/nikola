...
* Tested with this in GHCI:

  defaultMain $ do { x  <- newMArray (Z :. 10) :: P(MArray G (Data.Array.Nikola.Backend.CUDA.DIM1) (Data.Array.Nikola.Exp.Exp CUDA Double)); let y = unfoldPushArray (Z :. 10) (0 :: Data.Array.Nikola.Exp.Exp CUDA Double) (\ _ x -> x + 1) in loadP y x }

* Outputs: 

{ vec_alloca_0 :: Array Double[] <- alloc (Array Double[])[10]
; x_1 :: Double <- 0.0
; for((0) <= (i_2) < (10))
      { x_1 :: Double <- x_1 + 1.0
      ; write vec_alloca_0[0 * 10 + i_2] x_1
      ; return ()
      }
; return ()
}
*** Exception: user error (Expected monadic type but got Double)
-}
